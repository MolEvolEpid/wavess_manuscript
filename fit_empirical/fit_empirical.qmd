...
tr_indices_real <- lapply(names(trees_rooted), function(x){
  ...
  mutate(stat_name = factor(
    case_when(stat_name == 'divergence_slope' ~ 'Mean annual\nchange in divergence',
                               ...
                               stat_name == 'mean_ltt' ~ 'Mean number of\nlineages through time',
...
stats_supp <- all_stats %>% 
  mutate(type = case_when(grepl('ivergence|leaf', stat_name) ~ 'I+/S+',
                          grepl('iversity|bl|lineages', stat_name) ~ 'I+/S-',
...
inner_join(all_stats)  %>% 
  ...
  summarize(#min_sim = min(stat_value),
            ...
            mean_sim = mean(stat_value),
            sd_sim = sd(stat_value)) %>% 
  mutate(sd_away = abs(mean_sim - stat_value_real)/sd_sim) %>% 
  group_by(stat_name) %>% 
  summarize(sd_away_lt1 = sum(sd_away <= 1),
            sd_away_gt1 = sum(sd_away > 1),
            mean_sd_away = mean(sd_away),
            median_sd_away = median(sd_away),
            min_sd_away = min(sd_away),
            max_sd_away = max(sd_away))
