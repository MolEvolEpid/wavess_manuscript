...
lapply(c("mutation_rate", "recombination_rate", "conserved_cost", "replicative_cost", "immune_cost", "mean_fitness_cost"), function(x){
  ...
  annotate_figure(left = text_grob('Mean per-generation value across 10 replicates', rot = 90, size = 15))

ratio_fig <- ratio_summary %>% 
  left_join(prcc) %>% 
  mutate(stat_name = case_when(grepl('transitions', stat_name) ~ 'Mean number\nof lineages\nthrough time',
                               grepl('diversity', stat_name) ~ 'Mean annual\nchange in\ndiversity',
                               grepl('divergence', stat_name) ~ 'Mean annual\nchange in\ndivergence',
                               grepl('branch', stat_name) ~ 'Mean branch\nlength',
                               grepl('leaf', stat_name) ~ 'Mean leaf\ndepth',
                               TRUE ~ stat_name),
         which_diff = case_when(grepl('Days', which_diff) ~ 'Days to\nmaximum\nimmune cost', 
  ...
  geom_tile() +
  geom_text(col = 'white', lineheight = 0.9, size = 5) +
...
sens_fig <- cowplot::plot_grid(cowplot::plot_grid(NULL, counts_fig, nrow = 1, rel_widths = c(0.05, 1)), 
                               ratio_fig, rel_heights = c(1.25, 1), labels = 'AUTO', ncol = 1, label_y = c(1, 1.05), label_size = 15)

sens_fig

ggsave('figures/sensitivity_analysis.pdf', sens_fig, width = 17, height = 12)
```
